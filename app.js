...
function setupEventListeners() {
    ...
    const uploadCOIBtn = document.getElementById('uploadCOIBtn');
    const coiUploadForm = document.getElementById('coiUploadForm');
    const closeCOIUploadModalBtn = document.getElementById('closeCOIUploadModal');
    const cancelCOIUploadFormBtn = document.getElementById('cancelCOIUploadForm');
    const vendorCOIInput = document.getElementById('vendorCOIInput');
    
    if (uploadCOIBtn) {
        uploadCOIBtn.addEventListener('click', () => {
            let vendorId = currentVendorIdForDetail;
            if (!vendorId) {
                const vendorDetailModal = document.getElementById('vendorDetailModal');
                if (vendorDetailModal) {
    ...
    if (coiUploadForm) {
        coiUploadForm.addEventListener('submit', handleCOIUploadSubmit);
    }
    ...
    vendorTabButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            const tabName = this.getAttribute('data-tab');
            
            // Remove active class from all tabs and tab contents in vendor detail modal
            document.querySelectorAll('#vendorDetailModal .tab-btn').forEach(b => b.classList.remove('active'));
            document.querySelectorAll('#vendorDetailModal .tab-content').forEach(c => {
                c.classList.remove('active');
                c.style.display = 'none';
            });
            ...
            if (tabName === 'employees') {
                ...
                const vendorDetailModal = document.getElementById('vendorDetailModal');
                if (vendorDetailModal) {
                    const vendorId = vendorDetailModal.getAttribute('data-vendor-id') || currentVendorIdForDetail;
                    if (vendorId) {
                        loadEmployees(vendorId);
                    }
                }
            } else if (tabName === 'cois') {
                const coisTab = document.getElementById('coisTab');
                if (coisTab) {
                    coisTab.classList.add('active');
                    coisTab.style.display = 'block';
                }
                // Load COIs
                const vendorDetailModal = document.getElementById('vendorDetailModal');
                if (vendorDetailModal) {
    ...
    if (vendorDetailModal) {
        ...
        db.collection('vendors').doc(vendorId).get().then((doc) => {
            ...
            if (vendor) {
                ...
                const uploadCOIBtn = document.getElementById('uploadCOIBtn');
                if (uploadCOIBtn) {
                    uploadCOIBtn.style.display = canEditVendor() ? 'block' : 'none';
                }
            }
        });
        loadEmployees(vendorId);
        
        // Reset to employees tab
        document.querySelectorAll('#vendorDetailModal .tab-btn').forEach(b => b.classList.remove('active'));
        document.querySelectorAll('#vendorDetailModal .tab-content').forEach(c => {
            c.classList.remove('active');
            c.style.display = 'none';
        });
        const employeesTabBtn = document.querySelector('#vendorDetailModal .tab-btn[data-tab="employees"]');
        const employeesTab = document.getElementById('employeesTab');
        if (employeesTabBtn) employeesTabBtn.classList.add('active');
        if (employeesTab) {
            employeesTab.classList.add('active');
            employeesTab.style.display = 'block';
        }
...
// ============================================
// VENDOR COI (CERTIFICATE OF INSURANCE) MANAGEMENT
// ============================================

// Load vendor COIs
function loadVendorCOIs(vendorId) {
    const coisList = document.getElementById('coisList');
    if (!coisList) return;
        ...
        sortedCOIs.forEach((coi, index) => {
            ...
            coiCard.innerHTML = `
                ...
                <div class="contact-card-actions">
                    ...
                    ${canEditVendor() ? `
                        <button class="btn-danger btn-small" onclick="deleteVendorCOI('${vendorId}', ${index})">Delete</button>
                    ` : ''}
                </div>
            `;
...
// Upload vendor COI
async function uploadVendorCOI(vendorId, file, startDate, endDate, description) {
    try {
        const storageRef = firebase.storage().ref();
        const fileRef = storageRef.child(`vendors/${vendorId}/cois/${Date.now()}_${file.name}`);
        ...
        await new Promise((resolve, reject) => {
            uploadTask.on('state_changed',
                (snapshot) => {
                    const progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                    console.log(`COI upload progress: ${progress}%`);
                },
                (error) => {
...
// Delete vendor COI
window.deleteVendorCOI = async function(vendorId, coiIndex) {
...
function handleCOIUploadSubmit(e) {
    ...
    const coiForm = document.getElementById('coiUploadForm');
    if (!coiForm) {
        console.error('COI upload form not found');
        alert('Error: COI upload form not found. Please refresh the page.');
        resetButtonState();
        return;
    }
    
    const vendorId = coiForm.querySelector('#coiVendorId').value;
    const fileInput = coiForm.querySelector('#coiFile');
    const startDate = coiForm.querySelector('#coiStartDate').value;
    const endDate = coiForm.querySelector('#coiEndDate').value;
    const description = coiForm.querySelector('#coiDescription').value.trim() || null;
...
const COST_CODES_BY_COMPANY = {
    ...
    "CDC": [
        ...
        {"code": "1002", "description": "Customer Deposit"},
...
async function populateRentRollFilters() {
    ...
    try {
        ...
        propertyFilter.addEventListener('change', async () => {
            ...
            const buildingFilterEl = document.getElementById('rentRollBuildingFilter');
            if (buildingFilterEl) {
                const selectedPropertyId = propertyFilter.value;
                buildingFilterEl.innerHTML = '<option value="">All Buildings</option>';
                ...
                try {
                    ...
                    console.log(`✅ Loaded ${buildingsSnapshot.size} buildings for property: ${selectedPropertyId || 'All Properties'}`);
                    buildingsSnapshot.forEach(doc => {
                        const building = doc.data();
                        const option = document.createElement('option');
                        option.value = doc.id;
                    ...
                    try {
                        ...
                        if (selectedPropertyId) {
                            ...
                            buildingsSnapshot = await db.collection('buildings').get();
                        }
                        ...
                        console.error('❌ Error loading buildings (fallback):', error2);
                    }
                }
            }
            loadRentRoll();
        ...
        // Populate buildings on initial load
        const buildingFilterEl = document.getElementById('rentRollBuildingFilter');
        if (buildingFilterEl) {
            const selectedPropertyId = propertyFilter.value;
            buildingFilterEl.innerHTML = '<option value="">All Buildings</option>';
            ...
            try {
                ...
                console.log(`✅ Initial load: Loaded ${buildingsSnapshot.size} buildings for property: ${selectedPropertyId || 'All Properties'}`);
                buildingsSnapshot.forEach(doc => {
                    const building = doc.data();
                    const option = document.createElement('option');
                    option.value = doc.id;
                ...
                try {
                    ...
                    console.error('❌ Error loading buildings on initial load (fallback):', error2);
                }
            }
        }
        
        const buildingFilterChange = document.getElementById('rentRollBuildingFilter');
        if (buildingFilterChange) {
            buildingFilterChange.addEventListener('change', () => {
                loadRentRoll();
            });
        }
...
async function loadRentRoll() {
    ...
    try {
        ...
        // Group leases by property, then by tenant (combining active and deprecated leases for same tenant)
        const leasesByProperty = {};
        
        // First, combine all leases (active + deprecated) and group by tenant
        const allLeases = [...activeLeases];
        if (includeDeprecated) {
            allLeases.push(...deprecatedLeases);
        }
        ...
        allLeases.forEach(lease => {
            ...
            const tenantId = lease.tenantId || 'no-tenant';
            if (!leasesByProperty[propId][tenantId]) {
                leasesByProperty[propId][tenantId] = {
                    activeLeases: [],
                    deprecatedLeases: []
                };
            }
            ...
            // Categorize lease
            if (isLeaseDeprecated(lease)) {
                leasesByProperty[propId][tenantId].deprecatedLeases.push(lease);
            } else {
                leasesByProperty[propId][tenantId].activeLeases.push(lease);
            }
        });
...
// tenantsData: { tenantId: { activeLeases: [], deprecatedLeases: [] } }
function renderVerticalTable(tenantsData, tenants, units, buildings, year, monthLabels, buildingName = null, includeDeprecated = false) {
    let html = '';
    ...
    // Sort tenants by name
    const sortedTenantIds = Object.keys(tenantsData).sort((a, b) => {
        const tenantA = tenants[a]?.tenantName || '';
        const tenantB = tenants[b]?.tenantName || '';
    ...
    sortedTenantIds.forEach(tenantId => {
        ...
        // Get info from first active lease, or first deprecated lease if no active
        ...
        const squareFootage = firstLease.squareFootage || unit?.squareFootage || null;
    ...
    for (let month = 1; month <= 12; month++) {
        ...
        sortedTenantIds.forEach(tenantId => {
            ...
            // Process deprecated leases if included
            if (includeDeprecated) {
                tenantLeases.deprecatedLeases.forEach(lease => {
                    const result = calculateRentForMonth(lease, year, month);
                    const isDeprecated = isLeaseDeprecated(lease);
            ...
            if (allLeasesForMonth.length === 0) {
                ...
                allLeasesForMonth.forEach((leaseData, idx) => {
                    const rentDisplay = `$${leaseData.rent.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    ...
    sortedTenantIds.forEach(tenantId => {
        ...
        // Sum deprecated leases if included
        if (includeDeprecated) {
            tenantLeases.deprecatedLeases.forEach(lease => {
                for (let month = 1; month <= 12; month++) {
                    const result = calculateRentForMonth(lease, year, month);
...
// tenantsData: { tenantId: { activeLeases: [], deprecatedLeases: [] } }
function renderHorizontalTable(tenantsData, tenants, units, buildings, year, monthLabels, buildingName = null, includeDeprecated = false) {
    let html = '';
    ...
    sortedTenantIds.forEach(tenantId => {
        ...
        monthlyRentsByLease.forEach((allLeasesForMonth, idx) => {
            ...
            if (allLeasesForMonth.length === 0) {
                ...
                allLeasesForMonth.forEach((leaseData, leaseIdx) => {
                    const rentDisplay = `$${leaseData.rent.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                    const cellColor = leaseData.isDeprecated ? '#f87171' : '#1e293b';
                    const cellBgColor = leaseData.isDeprecated ? 'background-color: #fef2f2;' : '';
                    const leaseIdShort = leaseData.leaseId.substring(0, 8);
